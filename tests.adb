--  Standalone test suite for Kosarajus_Algorithm (main program).

pragma Ada_2022;

with Ada.Text_IO; use Ada.Text_IO;
with Kosarajus_Algorithm; use Kosarajus_Algorithm;
...
procedure Tests is
   ...
   begin
      ...
      Check (Comp (1) < Comp (4), "wiki triangle source before pair");
   end Test_Classic;
   ...
   -------------------------------------------------------------------------
   -- 7. Condensation / topological order (Kosaraju: sources first)
   -------------------------------------------------------------------------
   ...
   begin
      Section ("7. Condensation topological order (sources first)");

      --  A={1,2} → B={3}: Kosaraju assigns source A before sink B
      Clear (G, 3);
      Add_Edge (G, 1, 2);
      Add_Edge (G, 2, 1);
      Add_Edge (G, 2, 3);
      ...
      Check (Id_A < Id_B, "source SCC assigned first (smaller id)");

      --  Chain of three singleton SCCs 1→2→3: ids increase along the chain
      Clear (G, 3);
      Add_Edge (G, 1, 2);
      Add_Edge (G, 2, 3);
      Compute_SCC (G, Comp, Count);
      Check (Count = 3, "chain condensation 3");
      Check (Comp (1) < Comp (2) and then Comp (2) < Comp (3),
             "chain topo ids 1<2<3");
   ...
   begin
      ...
      Check (Comp (1) < Comp (3), "left source before right");
   ...
   -------------------------------------------------------------------------
   -- 14. Cross / forward / back edges (Kosaraju transpose walks)
   -------------------------------------------------------------------------
   ...
   procedure Test_Path_Contraction is
      ...
      Count : Natural;
   begin
      Section ("14. Cross / forward / back (transpose reachability)");
   ...
   begin
      ...
      Check (Comp (1) < Comp (4), "barbell left source before right");
      ...
      Check (Comp (1) < Comp (3) and then Comp (3) < Comp (5),
             "linked pairs topological");
   end Test_Nested;
   ...
   begin
      ...
      Check (Comp (1) < Comp (2) and then Comp (2) < Comp (5),
             "source-cycle-sink topological");
   ...
   -------------------------------------------------------------------------
   -- 21. Transpose invariance / finish-order intuition
   -------------------------------------------------------------------------

   procedure Test_Transpose_Invariance is
      G     : Graph;
      Comp  : Component_Array (1 .. 20);
      Count : Natural;
      Id_Src, Id_Mid, Id_Snk : Component_Id;
   begin
      Section ("21. Transpose invariance / finish-order");

      --  Reversing every edge yields the same SCC partition (not ids).
      Clear (G, 6);
      Add_Edge (G, 1, 2);
      Add_Edge (G, 2, 3);
      Add_Edge (G, 3, 1);
      ...
      Add_Edge (G, 5, 6);
      Compute_SCC (G, Comp, Count);
      Check (Count = 3, "forward classic → 3 SCCs");
      Check (Same_SCC (Comp, 1, 3), "fwd {1,2,3}");
      Check (Same_SCC (Comp, 4, 5), "fwd {4,5}");
      Check (not Same_SCC (Comp, 5, 6), "fwd 6 sink alone");
      ...
      Add_Edge (G, 6, 5);
      Compute_SCC (G, Comp, Count);
      Check (Count = 3, "transpose classic → 3 SCCs");
      Check (Same_SCC (Comp, 1, 3), "rev {1,2,3}");
      Check (Same_SCC (Comp, 4, 5), "rev {4,5}");
      Check (not Same_SCC (Comp, 5, 6), "rev 6 alone");

      --  Source → mid-cycle → sink: source gets smallest id.
      Clear (G, 5);
      Add_Edge (G, 1, 2);
      Add_Edge (G, 2, 3);
      Add_Edge (G, 3, 4);
      ...
      Id_Mid := Comp (2);
      Id_Snk := Comp (5);
      Check (Id_Src < Id_Mid and then Id_Mid < Id_Snk,
             "finish-order sources before sinks");

      --  Self-transpose 2-cycle: one SCC, id 1.
      Clear (G, 2);
      Add_Edge (G, 1, 2);
      Add_Edge (G, 2, 1);
      Compute_SCC (G, Comp, Count);
      Check (Count = 1 and then Comp (1) = 1, "2-cycle self-transpose");
   end Test_Transpose_Invariance;

begin
   Put_Line ("Kosarajus_Algorithm test suite");
   Put_Line ("=========================");
